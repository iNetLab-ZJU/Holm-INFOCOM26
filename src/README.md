# Parameters needed to build holm
...
Therefore, to go to a higher level, MBA_VAL_HIGH is written to an MSR, and to reduce to a lower level MBA_VAL_LOW is written to an MSR. One could change this logic by editing the *holm-local-response.c* file, and default high/low values by editing the *config.json* file.
...
If your server does not have 4 NUMA nodes, or your intend to use MApp on cores belonging to 1 or 2 NUMA nodes, you may have to create multiple COS IDs within a single NUMA node to achieve similar MBA response. This can be done by making minor modifications in the *holm-local-response.c* file in the methods *increase_mba_val()* and *decrease_mba_val()*.  

### Using process scheduling to overcome current hardware limitations

Current MBA implementation only introduces a fixed maximum hardware supported rate-throttling for CPU-memory traffic (when using the max possible value of 90 above). This may not be enough to achieve desired network throughput for the NetApp. Therefore we also provide an aditional MBA level 4 (inidicated by the flag USE_PROCESS_SCHEDULER) which takes as input PID for a process (see command line inputs below), and sends a SIGSTOP signal to the process when holm's local response requires going to the level 4. This is equivalent to completely throttling the rate at which the process cores generate memory traffic. When holm intends to go back to level 3, it then sends a SIGCONT signal to the process to resume its execution. Current implementation assumes that there is a single MApp process, and is available when holm module is inserted. 

+ "USE_PROCESS_SCHEDULER": Should be 1 if you intend to use holm's additional local response level,
...
+ "IIO_LOGGING": Set to 1 if desired to dump IIO occupancy log to the kernel message buffer upon exiting holm. Set to 0 otherwise.
+ "PCIE_CORE": Core used to measure PCIe bandwidth. Must be located local to the NUMA node containing the NIC.
+ "PCIE_LOGGING": Set to 1 if desired to dump PCIe bandwidth log to the kernel message buffer upon exiting holm. Set to 0 otherwise.
+ "ECN_LOGGING": Set to 1 if desired to ECN marking log to the kernel message buffer upon exiting holm. Set to 0 otherwise.
...
## Command-line parameters to run holm

holm uses following command line input parameters:
...
+ mode: 0 (default) means holm is running in Rx datapath mode, and 1 means holm is running in Tx datapath mode

### Building and Running holm

After configuring the config.json file, make holm by running make from within the src/ directory.
...
``` 
sudo insmod holm-module.ko mode=0 target_iio_wr_threshold=70 target_pcie_threshold=84
```
 
To run at the sender-side, use the following command:
 
``` 
sudo insmod holm-module.ko mode=0 target_iio_rd_threshold=190 target_pcie_threshold=84
```
 
to stop running holm at either side:
