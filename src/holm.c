#include "holm.h"
#include "holm-signals.h"
#include "holm-local-response.h"
#include "holm-network-response.h"
#include "holm-logging.h"
...
static int __init holm_init(void) {
  printk("Initializing holm");
  // create sysfs interface for taget_pcie_thresh
  my_kobj = kobject_create_and_add("holm", kernel_kobj);
  if (!my_kobj)
      return -ENOMEM;
...
static void __exit holm_exit(void) {
...
module_init(holm_init);
module_exit(holm_exit);
