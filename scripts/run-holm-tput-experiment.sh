...
help()
{
    echo "Usage: run-holm-tput-experiment
               ...
               [ -E | --exp (experiment name, this name will be used to create output directories; default='holm-test')]
               [ -M | --MTU (default=4000) ]
               ...
               [ --target_iio_thresh (holm specific parameter, target IIO occupancy threshold I_T; default = 70) ]
               [ --target_pcie_thresh (holm specific parameter, target PCIe threshold occupancy B_T; default = 84, which corresponds to 80Gbps app-level throughput) ]
               [ --target_pid (holm specific parameter, PID of the memory-intensive application process to which holm should perform process scheduling based host-local response, in case MBA response is less effective; by default we assume MLC is the app running and find its pid using pidof command) ]
               [ -h | --help  ]"
    exit 2
}
...
OPTS=$(getopt -a -n run-holm-tput-experiment --options $SHORT --longoptions $LONG -- "$@")
...
num_runs=2
home="/home/saksham"
holm_dir=$home/holm/src
setup_dir=$home/holm/utils
exp_dir=$home/holm/utils/tcp
...
function start_holm() {
    ...
    if [ "$mlc_cores" = "none" ]; then
        sudo insmod holm-module.ko target_pcie_thresh=$target_pcie_thresh target_iio_wr_thresh=$target_iio_thresh
    else
        target_pid=$(pidof mlc)
        echo "sudo insmod holm-module.ko target_pid=$target_pid target_pcie_thresh=$target_pcie_thresh target_iio_thresh=$target_iio_thresh"
        sudo insmod holm-module.ko target_pid=$target_pid target_pcie_thresh=$target_pcie_thresh target_iio_wr_thresh=$target_iio_thresh
    fi
    sleep 10 #give time for holm module to load 
    cd -
}

function fin_holm() {
    local holm_log_dir=$1
    cd $holm_dir
    sudo rmmod holm-module.ko
    sleep 15
    sudo dmesg > $holm_log_dir/hcc.log
    cd -
...
#post-run cleanup
fin_holm $setup_dir/logs/$exp-RUN-$j
cleanup
done
...
if [ "$mlc_cores" = "none" ]; then
    ...
    #### start holm
    sleep 5  #give a few seconds for MLC to run, so that holm can register the MLC PID
    start_holm
    ## wait until MLC starts sending memory traffic at full rate
    sleep 60
    echo "Running MLC..."
    ...
    #post-run cleanup
    fin_holm $setup_dir/logs/$exp-MLCRUN-$j
    cleanup
    done
fi
