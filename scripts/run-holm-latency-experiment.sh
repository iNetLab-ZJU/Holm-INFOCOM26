...
help()
{
    echo "Usage: run-holm-latency-experiment
...
OPTS=$(getopt -a -n run-holm-latency-experiment --options $SHORT --longoptions $LONG -- "$@")
...
function start_holm() {
    ...
    if [ "$mlc_cores" = "none" ]; then
        sudo insmod holm-module.ko target_pcie_thresh=$target_pcie_thresh target_iio_thresh=$target_iio_thresh
    else
        ...
        sudo insmod holm-module.ko target_pid=$target_pid target_pcie_thresh=$target_pcie_thresh target_iio_thresh=$target_iio_thresh
    fi
...
#post-run cleanup
fin_holm $setup_dir/logs/$exp-LATRUN-$j
cleanup
done
